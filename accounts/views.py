...
def reporthome(request):
    return render(request,'accounts/reporthome.html')

def reportabout(request):
    return render(request,'accounts/reportabout.html')

def reportanalyse(request):
    ...
    if(0 <= num <=100):
        ...
        try:
            ...
            if request.method == "POST":
                return render(request,'accounts/reportanalyse.html',{'p':filePathName,'file':file_name,'sheet':sheet,'start':start_date.date(),'end':end_date.date(),'teacher':first_output.to_html(),'student':df.to_html(),'less_than_25':less_than_25.to_html(),
